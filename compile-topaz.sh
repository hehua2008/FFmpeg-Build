...
echo '♻️ ' Start compiling FFMPEG-Topaz

cd ${COMPILED}

cd ffmpeg-topaz

git am ${COMPILED}/Fix-frame_rate-for-tvai.patch

cp -r ${COMPILED}/avbuild/patches-master patches-master

rm patches-master/0025-wolfSSL-new-TLS-backend-patch.patch

git am patches-master/*.patch

export LDFLAGS="-L${SOURCE}/lib -L${COMPILED}/ffmpeg-redist/lib"

export CFLAGS="-I${SOURCE}/include -I${COMPILED}/ffmpeg-redist/include/videoai"

export LDFLAGS="$LDFLAGS -framework VideoToolbox"

./configure --prefix=${SOURCE} --extra-cflags="-fno-stack-check" --arch=arm64 --cc=/usr/bin/clang --enable-gpl --enable-libbluray --enable-libopenjpeg --enable-libopus --enable-libmp3lame --enable-libx264 --enable-libx265 --enable-libvpx --enable-libwebp --enable-libass --enable-libfreetype --enable-fontconfig --enable-libtheora --enable-libvorbis --enable-libsnappy --enable-libaom --enable-libvidstab --enable-libzimg --enable-libsvtav1 --enable-libkvazaar --enable-libharfbuzz --pkg-config-flags=--static --enable-sdl2 --enable-ffplay --enable-postproc --enable-neon --enable-runtime-cpudetect --enable-tvai --disable-indev=qtkit --disable-indev=x11grab_xcb
